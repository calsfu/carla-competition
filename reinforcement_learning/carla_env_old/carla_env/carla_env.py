...
class CarlaEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.lane_invasion_sensor = self.world.spawn_actor(sensor_blueprint, carla.Transform(), attach_to=self.vehicle)
        ...
        for _ in range(self.frame_skip):
            if self.autopilot:
                ...
                if vehicle_control.throttle > 0.0 and vehicle_control.brake == 0.0:
                    ...
                    throttle_brake = 0.0
                action = [steer, throttle_brake, 0.0]
    ...
    def _simulator_step(self, action):
        ...
        # convet image from 320x240x3 to  3x320x240
        next_obs = np.transpose(next_obs, (2, 0, 1))
        return next_obs, reward, done, info

    def _get_pixel_obs(self, vision_image):
        bgra = np.array(vision_image.raw_data).reshape(320, 240, 4)
        bgr = bgra[:, :, :3]
    ...
    def _get_reward(self):
        ...
        target_speed_reward = self._get_follow_speed_reward(vehicle_speed)
        done_collision, collision_reward = self._get_collision_reward()
        done_lane, lane_invasion_reward = self._get_lane_invasion_reward()

        # some bugs with lane invasion w/ spawning vehicles
        # lane_invasion_reward = 0
        # done_lane = False

        done = done_collision or done_lane
        cost = self._get_cost()
        total_reward = 100 * follow_waypoint_reward + collision_reward + lane_invasion_reward + 1000 * target_speed_reward

        info_dict = dict()
        info_dict['follow_waypoint_reward'] = follow_waypoint_reward
        info_dict['collision_reward'] = collision_reward
        info_dict['lane_invasion_reward'] = lane_invasion_reward
        info_dict['cost'] = cost
        info_dict['target_speed_reward'] = target_speed_reward

        # print(info_dict)

        return total_reward, done, info_dict
    
    def _get_follow_speed_reward(self, speed):
        speed_reward = 1.0 - abs(speed - self.target_speed) / self.target_speed
    ...
    def _get_lane_invasion_reward(self):
        if not self.lane_invasion:
            return False, 0
        else:
            return True, -10.0
